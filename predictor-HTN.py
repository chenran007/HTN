...
if st.button("Predict"):
    ...
    # 根据预测结果生成建议
    # 如果预测类别为 1（高风险）
    if predicted_class==1: # float(predicted_proba[1])>explainer_shap.expected_value:  # 修改点，不能这样判断 float(predicted_proba[1])>explainer_shap.expected_value, 如果要通过概率进行分割， 需要设置阈值
    ...
    # 特征值数据
    # 使用 Matplotlib 绘图
    shap.force_plot(explainer_shap.expected_value, shap_values, pd.DataFrame([feature_values], columns=feature_names), matplotlib=True)  # 修改点: shap_values,  explainer_shap.expected_value  不要带索引， 版本不同只有一个值
    # 期望值（基线值）
    # 解释类别 0（未患病）的 SHAP 值
    # 特征值数据
    # 使用 Matplotlib 绘图 
